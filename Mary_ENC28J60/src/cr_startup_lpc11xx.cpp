...
#if 0
__attribute__ ((section(".after_vectors")))
void call_static_initializers( unsigned int startaddr, unsigned int endaddr )
{
	void (*static_ctor)() = (void(*)())( startaddr );
	for( unsigned int addr = startaddr; addr < endaddr; addr += 4 ){
		static_ctor();
	}
}
#endif
